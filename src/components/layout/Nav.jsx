...
// 4 items around the central FAB (mobile)
  ...
  return (
    <nav className="sm:hidden fixed bottom-0 left-0 right-0 z-40 glass-dark border-t border-white/5 shadow-2xl">
      <div className="flex items-center justify-around px-1 py-1.5 pb-safe relative">
        ...
        {/* Center FAB */}
        <div className="flex flex-col items-center relative z-10" style={{ marginTop: -20 }}>
          <motion.button
            whileTap={{ scale: 0.88 }}
          ...
          >
            <Dumbbell size={21} className="text-white drop-shadow-lg" />
          </motion.button>
          {streak > 0 ? (
            <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }}
              className="flex items-center gap-0.5 mt-0.5">
              <span className="text-[9px] animate-fire">🔥</span>
              <span className="text-[8px] font-black text-orange-400">{streak}</span>
            </motion.div>
          ) : (
            <span className="text-[7px] text-slate-600 font-bold mt-0.5 uppercase tracking-wide">Séance</span>
          )}
        </div>
        ...
        {/* Admin Shield for Admin on Mobile */}
        {profile?.role === 'admin' && (
          <NavLink to="/admin"
            className={({ isActive }) =>
              `flex flex-col items-center gap-0.5 px-2 py-1.5 rounded-xl transition-all duration-200 min-w-[48px] ${
          ...
          >
            {({ isActive }) => (
              <>
                <div className="relative">
                  <Shield size={19} className={isActive ? "text-emerald-400 drop-shadow-[0_0_6px_currentColor]" : "text-emerald-500"} />
                  {isActive && (
                    <motion.div layoutId="bottom-indicator"
                      className="absolute -inset-1.5 bg-emerald-500/15 rounded-xl -z-10"
                      transition={{ type: "spring", bounce: 0.25, duration: 0.35 }}
                    />
                  )}
                </div>
                <span className={`text-[7px] font-bold uppercase tracking-wider ${isActive ? "text-emerald-400" : "text-emerald-500"}`}>Admin</span>
              </>
            )}
          </NavLink>
        )}
